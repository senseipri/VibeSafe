...
class DBConfigScanner(BaseScanner):
    ...
    async def scan(self, manifest: RepoManifest) -> list[Finding]:
        ...
        # Determine whether this project is Supabase-based before scanning SQL files.
        # We need this to avoid false-positives on plain PostgreSQL schemas.
        supabase_project = self._is_supabase_project(manifest)
        ...
        for file_path in manifest.files:
            ...
            if suffix == ".sql":
                findings.extend(self._scan_sql(file_path, rel, supabase_project))
    ...
    def _is_supabase_project(self, manifest: RepoManifest) -> bool:
        """
        Return True when the manifest gives strong evidence this is a Supabase
        project.  Checked in order of specificity:

        1. A file lives under a ``supabase/`` directory.
        2. ``package.json`` / ``pyproject.toml`` / ``requirements*.txt`` lists
           ``@supabase/supabase-js``, ``supabase-py``, or ``supabase``.
        3. A ``.env`` or config file references ``SUPABASE_URL`` /
           ``SUPABASE_ANON_KEY``.
        4. A SQL file itself mentions ``supabase`` (e.g. a comment header).
        """
        supabase_dep_re = re.compile(
            r"supabase", re.IGNORECASE
        )
        # In SQL files, the bare word "supabase" in a comment is NOT sufficient;
        # we require functional Supabase identifiers (auth.uid, service_role, etc.)
        supabase_sql_re = re.compile(
            r"(?:auth\.uid\(\)|supabase_url|supabase_anon_key|service_role"
            r"|storage\.foldername|storage\.filename|auth\.role\(\)|auth\.jwt\(\))",
            re.IGNORECASE,
        )
        for file_path in manifest.files:
            rel = manifest.relative(file_path)
            # Path-level signal
            if "supabase" in rel.lower().replace("\\", "/"):
                return True
            # Dependency-level signal
            if file_path.name in (
                "package.json", "requirements.txt", "requirements-dev.txt",
                "pyproject.toml",
            ):
                content = self._read(file_path)
                if content and supabase_dep_re.search(content):
                    return True
            # Environment / config signal
            if file_path.suffix.lower() in (".env", ".cfg", ".ini", ".toml", ".yaml", ".yml"):
                content = self._read(file_path)
                if content and ("SUPABASE_URL" in content or "SUPABASE_ANON_KEY" in content):
                    return True
            # SQL file inline signal — require a functional Supabase identifier,
            # not just the word "supabase" in a comment.
            if file_path.suffix.lower() == ".sql":
                content = self._read(file_path)
                if content and supabase_sql_re.search(content):
                    return True
        return False

    # ── Supabase SQL migrations ─────────────────────────────────

    def _scan_sql(self, file_path: Path, rel: str, supabase_project: bool = False) -> list[Finding]:
        findings: list[Finding] = []
        content = self._read(file_path)
        if not content:
            return findings

        # RLS checks are only meaningful on Supabase projects.
        # On a plain PostgreSQL schema (Django, SQLAlchemy, Alembic, etc.) there
        # is no anon-key threat model, so skip completely.
